...
/**
 * This file replaces /theme/styles.php for moodleorg theme
 *
 ...
 * for example the file "theme/1548946762/moodleorg/css/all-3_1548750271.css"
...
if (preg_match('/all-(\w+)-?(rtl)?$/', $type, $matches)) {
    $preset = $matches[1];
...
if ($sendaftergeneration || $lock) {
    ...
    if (!file_exists($chunkedcandidatesheet)) {
        ...
        $candidatesheet = theme_styles_generate_and_store($theme, $rev, $themesubrev, $candidatedir, $type);
    }
 ...
 * @return  string
 */
function theme_styles_generate_and_store($theme, $rev, $themesubrev, $candidatedir, $type = null) {
    global $CFG;
...
function theme_styles_fallback_content($theme) {
    ...
    if (!empty($theme->settings->preset)) {
        $type = 'all-' . $theme->settings->preset . ($theme->get_rtl_mode() ? '-rtl' : '');
